...
sys.path.append("/home/matthew/university_work/crucial_1000gb/python_stuff/python_scripts")
...
dependency_paths = {#'syinterferopy'        : '/home/matthew/university_work/crucial_1000gb/15_my_software_releases/SyInterferoPy-3.0.1/',              # Available from Github: https://github.com/matthew-gaddes/SyInterferoPy
                    'syinterferopy'         : '/home/matthew/university_work/crucial_1000gb/01_blind_signal_separation_python/SyInterferoPy',
                    'srtm_dem_tools'        : '/home/matthew/university_work/crucial_1000gb/15_my_software_releases/SRTM-DEM-tools-2.1.1/',                # Available from Github: https://github.com/matthew-gaddes/SRTM-DEM-tools
                    'volcnet'               : '/home/matthew/university_work/crucial_1000gb/13_volcnet',                                                   # Available from Github: https://github.com/matthew-gaddes/VolcNet
                    'deep_learning_tools'   : '/home/matthew/university_work/crucial_1000gb/20_deep_learning_tools'}                                      # Available from Github: https://github.com/matthew-gaddes/Deep-Learning-Tools
...
#step 03 (create real interferograms from VolcNet data):
...
n_files_test = 2                                                                                # to be used for testing

#setp 04: Augment real data
augmentation_factor = 2                                                                        # factor to augment by.  E.g. if set to 10 and there are 30 data, there will be 300 augmented data.

# step 05 (merge synthetic and real, and rescale to desired range)
cnn_settings = {'input_ranges'       : [{'min':-1, 'max':1},                                # used by VGG16, ResNet, Inception
                                       # {'min':0, 'max':1},                                 # 
                                       {'min':0, 'max':255}]}                               # used by Efficeintnet
...
# # outdir = Path("/home/matthew/university_work/crucial_1000gb/02_neural_network_python/03_VUDLNet_21_github_repo/02_full_size_example/step_02_synthetic_data_2")
...
# #%%  Step 03: Create real data (interferograms) from the VolcNet data
...
# #%% Step 04: Augment the real (VolcNet) data.  
...
for input_range in cnn_settings['input_ranges']:
    print(f"Saving the data with the range rescaled to {input_range}")

    outdir = project_outdir / f"step_05_merged_rescaled_data_range_{input_range['min']}_{input_range['max']}"
    merge_and_rescale_data(synthetic_data_files, real_data_files, outdir,
                           input_range, triplicate_channel = True)                                          # merge the real and synthetic data, and rescale it into the correct range for use with the CNN

    open_datafile_and_plot(outdir / "data_file_00000.npz", n_data = 15, window_title = f"05 Sample of merged and rescaled data (range {input_range['min']} {input_range['max']})")
...
for input_range in cnn_settings['input_ranges']:
    ...
    outdir = project_outdir / f"step_05a_merged_rescaled_data_synth_only_range_{input_range['min']}_{input_range['max']}"
    rescale_data(synthetic_data_files, outdir, input_range, triplicate_channel = True)
    
    open_datafile_and_plot(outdir / "data_file_00000.npz", n_data = 15, window_title = f"05 Sample of merged and rescaled syntehtic data (range {input_range['min']} {input_range['max']})")
