...
def plot_data_class_loc_caller(X_m, classes=None, locs=None, classes_predicted=None, locs_predicted=None, 
                               source_names = None, point_size=5, figsize = (6,4)):
    ...
    if n_plots == 1:
        ...
        for n_plot in np.arange(n_plots-1):
            ...
            plot_data_class_loc(X_m, plot_args, classes, locs, classes_predicted, locs_predicted, source_names, point_size, figsize)
...
def plot_data_class_loc(data, plot_args, classes=None,           locs=None, 
    ...
    Inputs: 
        ...
        locs | r2 array | locations of deforamtion, columns are x, y, x half width, y half width
    Returns:
        Figure
    History:
        2019/??/?? | MEG | Written
    ...
    from neural_network_plotting_functions import centre_to_box
    from small_plot_functions import add_square_plot, remappedColorMap
    ...
    for n_plot in range(n_plots):                                           # loop through each plot arg
        ...
        ifg_max = ma.max(data[plot_args[n_plot], :,:,0])
        ifg_mid = 1 - ifg_max/(ifg_max + abs(ifg_min))
        
        
        cmap_scaled = remappedColorMap(cmap_noscaling, start=0, midpoint=ifg_mid, stop=1.0, name='cmap_noscaling')
        axe.imshow(data[plot_args[n_plot], :,:,0], cmap = cmap_scaled)
        ...
        if np.abs(ifg_max) > np.abs(ifg_min):
            cb2.ax.xaxis.set_ticks([np.round(0,0), np.round(ifg_max, 1)])
        else:
            cb2.ax.xaxis.set_ticks([np.round(ifg_min,1), np.round(0,0)])       
