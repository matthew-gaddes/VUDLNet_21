...
from vudlnet21.neural_net import define_two_head_model, file_list_divider, file_merger, numpy_files_sequence, save_model_each_epoch, save_batch_loss
...
block5_lr = 1.0e-7                                                                  # a pretty important parameter.  We have to set a learning rate manually as an adaptive approach (e.g. NADAM) will be high initially, and therefore make large updates that will wreck the model (as we're just fine-tuning a model so have something good to start with)
